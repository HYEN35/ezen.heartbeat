...
public class PageDTO {
	...
	private int num; // 현재 페이지 번호 : pageCalc()에서 사용하므로 사용자가 페이지 번호 버튼을 클릭할 때 현재 페이지 번호를 Controller에서
						// 수집해서 설정한다.
	...
	private void pageCalc() {
		...
		// num이 12이므로 endPageNum은 15이고, startPageNum은 11이다. 따라서 게시물 번호 136번의 블럭 페이지 번호는
		// 3번이다.
		endPageNum = (int) (Math.ceil((double) num / (double) pageNumCnt)) * pageNumCnt;
		startPageNum = endPageNum - (pageNumCnt - 1);

		// 게시물 번호가 136번이 마지막 게시물인 경우 페이지 개수는 11.3개가 필요하므로 페이지 번호는 올림처리하여 페이지 번호 버튼은
		// 12번까지 필요하다. 하지만 페이지 번호 버튼은 15번까지 출력된다. 따라서 블럭 페이지의 마지막 페이지 번호에 대한 재계산이 필요하다.
