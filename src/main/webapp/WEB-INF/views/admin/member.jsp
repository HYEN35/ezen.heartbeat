						...
						<div class="midCnt">
							...
							<c:forEach items="${urList}" var="uvo">
								<li class="item">
									...
									<div class="btnWrap">
										<a href="/admin/edit?email=${uvo.email}&num=${select}&searchType=${searchType}&keyword=${keyword}&role_id=${role_id}" class="btn-border">수정</a>
							...
							<div class="pagination">
								<ul class="page">
									<c:if test="${page.prev}">
									<li><a href="/admin/member?num=${page.startPageNum - 1}&searchType=${searchType}&keyword=${keyword}&role_id=${role_id}" class="btn-i-prev"><i class="bi bi-chevron-left"></i></a></li>
									</c:if>
									...
									<li>
										...
										<a href="/admin/member?num=${num}&searchType=${searchType}&keyword=${keyword}&role_id=${role_id}" class="num">${num}</a>
										</c:if>
										<c:if test="${select == num}">
										<a href="/admin/member?num=${num}&searchType=${searchType}&keyword=${keyword}&role_id=${role_id}" class="num on">${num}</a>
										</c:if>
									</li>
									</c:forEach>
									
									<c:if test="${page.next}">
									<li><a href="/admin/member?num=${page.endPageNum + 1}&searchType=${searchType}&keyword=${keyword}&role_id=${role_id}" class="btn-i-next"><i class="bi bi-chevron-right"></i></a></li>
									</c:if>
								</ul>
							</div>
						</div>
