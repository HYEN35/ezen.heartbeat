...
<body>	
			...
			<div class="cntWrap">
				...
				<div class="cntArea">
					<div class="section-banner">
						<img src="${pageContext.request.contextPath}/img/artist/blackpink-header.jpg" alt="newjeans" class="thumb">
					</div>
					<div class="section-artist-post">
						<div class="artistWrap">
							<div class="slideBx">
								<c:forEach items="${blackpinkPosts}" var="blackpinkVO">
									<div class="postBx">
										<form id="postFrm_${blackpinkVO.post_id}" action="/community/getPost" method="post">
							                <input type="hidden" name="post_id" value="${blackpinkVO.post_id}"/>
											<a href="javascript:void(0);" onclick="popPostArtistShow('${blackpinkVO.post_id}','${UserVO.email }');">
												<div>
													<div class="arti-profile">
														<c:if test="${blackpinkVO.profileimg != null && blackpinkVO.profileimg != ''}">
														    <img src="/heartbeat-upload/${blackpinkVO.profileimg}" alt="닉네임" class="arti-thumb">
														</c:if>
														<c:if test="${blackpinkVO.profileimg == null || blackpinkVO.profileimg == ''}">
													...
													<div class="arti-comment">
														<div class="arti-top">
															...
															<span class="arti-name">${blackpinkVO.nickname }</span>
														</div>
														<div class="arti-cnt">
															<div class="txt">${blackpinkVO.content }</div>
														</div>
													</div>
												</div>
											</a>
					...
					<div class="section-fan-post">
						<div class="fanWrap">
							...
							<div class="postWrap">
								<c:forEach items="${blackpinkFanPosts}" var="PostVO">
									<div class="postBx">
													...
													<div class="fan-comment">
														<div class="fan-cnt">
															<c:if test="${not empty PostVO.post_img}">
																<img src="/heartbeat-upload/${PostVO.post_img}" alt="게시판 이미지" style="width:100%;"><br><br> 	
															</c:if>
						...
						<div class="pagination">
							<c:if test="${page.prev }">
							<a href="/community/artist/blackpink]?email=${UserVO.email }&num=${page.startPageNum-1 }" class="btn-i-prev"></a>
							</c:if>
							<div class="page">
								<c:forEach begin="${page.startPageNum }" end="${page.endPageNum }" var="num">
									<c:if test="${select != num}">
									<a href="/community/artist/blackpink?email=${UserVO.email }&num=${num }" class="num">${num }</a>
									</c:if>
									<c:if test="${select == num}">
									<a href="/community/artist/blackpink?email=${UserVO.email }&num=${num }"class="num on">${num }</a>
									</c:if>
								</c:forEach>							
							</div>
							<c:if test="${page.next }">
							<a href="/community/artist/blackpink?email=${UserVO.email }&num=${page.endPageNum+1 }" class="btn-i-next"></a>
							</c:if>
						</div>
					</div>
				</div>
	...
	<!-- [D] 팝업 포스트작성 -->
	<div class="popup pop-post blackpink"><%@ include file="../../popup/pop-post-blackpink.jsp" %></div>
</body>
</html>