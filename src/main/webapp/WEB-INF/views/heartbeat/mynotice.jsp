						...
						<div > <!-- class="cntBx tab-post" -->
							<div class="cnt">
								...
								<div class="pagination">
									<c:if test="${page.prev }">
										<a href="/mynotice?num=${page.startPageNum-1 }" class="btn-i-prev"></a>
										</c:if>
											<div class="page">
												<c:forEach begin="${page.startPageNum }" end="${page.endPageNum }" var="num">
												<c:if test="${select != num}">
												<a href="/mynotice?num=${num }" class="num">${num }</a>
												</c:if>
												<c:if test="${select == num}">
												<a href="/mynotice?num=${num }" class="num on">${num }</a>
												</c:if>
												</c:forEach>
											</div>
										<c:if test="${page.next }">
										<a href="/mynotice?num=${page.endPageNum+1 }" class="btn-i-next"></a>
									</c:if>
								</div>
							</div>
						</div>
