	...
	<script>
		$(function(){
			...
			$('#search-btn1').click(function(){
				...
				location.href="/mynotice?&num=1&searchType="+searchType+"&keyword="+keyword;
			});
						...
						<div > <!-- class="cntBx tab-post" -->
							<div class="cnt">
								    ...
								    <ul class="itemWrap">
								    	<c:forEach items="${userNoticeList}" var="userNotice">
								            <li class="item">
								                ...
								                <a href="/getMyPostOne?notice_id=${userNotice.notice_id }&num=${select}&searchType=${searchType}&keyword=${keyword}" class="tit"><i>${userNotice.title}</i></a>
								            </li>
								    	</c:forEach>
								    </ul>
								</form>
								<div class="pagination">
									<c:if test="${page.prev }">
										<a href="/mynotice?num=${page.startPageNum-1 }&searchType=${searchType}&keyword=${keyword}" class="btn-i-prev"></a>
										</c:if>
											<div class="page">
												<c:forEach begin="${page.startPageNum }" end="${page.endPageNum }" var="num">
												<c:if test="${select != num}">
												<a href="/mynotice?num=${num }&searchType=${searchType}&keyword=${keyword}" class="num">${num }</a>
												</c:if>
												<c:if test="${select == num}">
												<a href="/mynotice?num=${num }&searchType=${searchType}&keyword=${keyword}" class="num on">${num }</a>
												</c:if>
												</c:forEach>
											</div>
										<c:if test="${page.next }">
										<a href="/mynotice?num=${page.endPageNum+1 }&searchType=${searchType}&keyword=${keyword}" class="btn-i-next"></a>
									</c:if>
								</div>
							</div>
						</div>
